...
if (!empty($_GET)) {
    ...
    $estado = $_GET['estado'];
    header("Location: puerto_ticket_proceso.php?server_port=$numero_puertos&estado=$estado");
    exit();
}
...
{	
    ...
    $html .= "\n";
    $html .= "$t3<th>NOMBRE</th>\n";
    $html .= "$t3<th>PUERTO</th>\n";
    $html .= "$t3<th>ESTADO</th>\n";
    #$html .= "$t3<th>vacio 2</th>\n";
    $html .= "$t2</tr>\n";
    $html .= "$t1</thead>\n";
    $html .= "$t1<tbody>\n";
    foreach ($array_puertos as $key => $value) {
        $numero_puerto = $value['ws_port'];
        $nombre_puerto = $value['name'];
        $estado_puerto = (in_array($numero_puerto, $array_puertos_activos)) ? 'activado' : 'desactivado';
        //$shelf = $value['shelf'];
        $html .= "$t2<tr>\n";
        $html .= "$t3<td class='$estado_puerto'><input type='checkbox' class= 'puertos' name='puertos[]' value=".$numero_puerto." ></td>\n";
        $html .= "$t3<td align ='center' class='$estado_puerto'>".$nombre_puerto."</td>\n";
        #$html .= "$t3<td align ='center' class='$estado'>".$shelf."</td>\n";
        $html .= "$t3<td align ='center' class='$estado_puerto'>".$numero_puerto."</td>\n";

        $html .= "$t3<td  class='$estado_puerto' align ='center'> Puerto $estado_puerto </td>\n";
...
?>
        <form name="formulario_on_off_puertos"
            ...
            action="puerto_ticket_proceso.php"
            method="POST">
